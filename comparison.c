...
uint64_t get_n_highest_cards(uint64_t hand, int n, int start_index, int jump_size)
{
    uint64_t ret = 0;
    int card_count = 0;
    ...
    do
    {
        ...
        card_count += (hand & (1 << bit_shift)) >> bit_shift; //the inner bit_shift gets the [bit_shift]th bit of [temp_mask], the outer bit_shift brings it to either zero or one
        ret += extracted_bit << card_index;
        bit_shift -= jump_size;
        card_index--;
    } while(card_count < n); //when we get the top 5 flush cards, return
    return (hand >> bit_shift) << bit_shift; //drop all the 1's below the selected top 5 cards and return
}
    ...
    for(int i = 9; straight_hand > indicator; indicator = indicator << 1, i--) //[i] counts the lowest card in the straight
    {
        if(bitwise_contains((uint64_t)straight_hand, indicator))
        {
            ...
            if(!mask || !straight_flush && sf) //if there's no straight yet, or if there is, but not a straight flush
            {
            	...
            	straight_high_card = i; //store the high card in the flush
	    	straight_flush_found = sf; //record whether this is a straight flush
	    }
        }
    }
    return (mask | (straight_high_card << STRAIGHT_BIT_OFFSET) | ((uint64_t)straight_flush_found << STRAIGHT_FLUSH_BIT_OFFSET)); //create the score from a) the five cards and b) the straight flush indicator (seeing this makes the rater return immediately)
    return 0LL;
}
    ...
    for(uint64_t hand = full_hand; hand > 2 ; hand = hand << 4, i--) //[i] keeps track of what card we're on, hand always has the [i]th card in the rightmost 4 bits | [i] starts at 2 so that pairs of aces are considered pairs of 14s, not pairs of 1s
    {
        ...
        if(indicator > 2)
        {
            if(indicator == 3 || indicator == 5 || indicator == 6 || indicator == 9 || indicator == 10 || indicator == 12) //checking equality to 0011, 0101, 0110, 1001, 1010, 1100
            {
                ...
                {
                    high_pairs[0] = i; //then this pair is highest pair
                }
                else if(high_pairs[0] && !high_pairs[1]) //if there's already a highest pair, but no second highest
                {
                    high_pairs[1] = i; //then this pair is second highest pair
                }
            ...
            if(indicator == 15)
            {
                return ((uint64_t)i << FOUR_OF_A_KIND_BIT_OFFSET) | (1 << i); //if we find a 4-of-a-kind, stop; there's no better multiples to be found  | return the index of the 4-of-a-kind and the indicator bit
            }
        }
    }
